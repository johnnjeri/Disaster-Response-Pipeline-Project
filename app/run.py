...
def index():
    ...
    #Get most popular words in the messages
    word_series = pd.Series(' '.join(df['message']).lower().split())
    top_words = word_series[~word_series.isin(stopwords.words("english"))].value_counts()[:5].sort_values(ascending=False)
    ...
    graphs = [
        ...
        },
        {
            ...
            'layout': {
                'title': 'Distribution of Target Variables',
                'yaxis': {
                    'title': "Count"
                },
                'xaxis': {
